...
import { DevicesLastRead } from '../types/DevicesLastRead.js';
...
export class TelemetryRepository {
  async latestAc(deviceId: string) {
    const result = await db.query<{
      ...
      FROM telemetry_ac WHERE device_id = $1::uuid ORDER BY time DESC LIMIT 1`,
      [deviceId],
    );
  ...
  async latestEnv(deviceId: string) {
    const result = await db.query<{
      ...
      WHERE device_id = $1::uuid ORDER BY time DESC LIMIT 1`,
      [deviceId],
    );
    return result.rows[0] ?? null;
  }
  ...
  async generation(deviceId: string, period: Period, userId: string) {
    return db.query<{ label: string; value: string }>(
      'SELECT * FROM public.get_generation_v2($1::uuid, $2::text, $3::uuid)',
  ...
  async efficiency(acDeviceId: string, dcDeviceId: string, period: Period) {
    return this.aggregate(
      ...
      CROSS JOIN LATERAL (SELECT COALESCE(SUM(generation_kwh),0::numeric) dc_kwh, COALESCE(SUM(sample_count),0::bigint) sample_count FROM telemetry_dc_hourly WHERE device_id=$2::uuid AND bucket >= b.period_start AND bucket < b.period_end) dc`,
